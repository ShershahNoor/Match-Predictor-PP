...
def get_trained_model_and_stats(_league_id, _season):
    ...
    if len(X) == 0:
        st.warning(f"Not enough completed match data in {_season} to train the model for this league.")
        return None, stats_map, teams
...
st.title("Tuffy Blud ShershahAI Match Predictor 🥶")
st.write("Data gathered from API-Sports, Deep Learning Personal Project")

tabs = st.tabs(list(LEAGUES.keys()))

def display_results(home, away, probs):
    """Helper function to render the 'Personality' and the stats."""
    ...
    with c2:
        st.metric("AI Confidence", f"{confidence:.1f}%", delta="High" if confidence > 60 else "Low")

    st.write("### Probability Gauge")
    st.progress(float(probs[idx])) 

    if idx == 0:
        st.success(f"**{home}** has the home pitch advantage ShershahAI predicts celebration.")
    elif idx == 2:
        st.success(f"**{away}** is looking like Messi 2012 on the road. ShershahAI predicts heartbreak.")
    else:
        st.warning(f"⚖️ The two are too evenly matched. ShershahAI figures a draw.")

def render_league_tab(name, L_ID):
    """Everything inside this function stays isolated to its specific tab."""
    st.header(f"{name} Predictor")
    
    with st.spinner(f"Scouting {name} teams..."):
        model, stats_map, team_dict = get_trained_model_and_stats(L_ID, PREFERRED_SEASON)

    if not team_dict:
        st.error("Could not load teams. Check your API key or connection.")
        return
    ...
    if st.button(f"Analyze Matchup", key=f"btn_{L_ID}", type="primary"):
        if home == away:
            st.error("You can't play against yourself, gng.")
        else:
            home_id = team_dict[home]
            away_id = team_dict[away]

            if home_id not in stats_map or away_id not in stats_map:
                st.error("Stats missing for these teams in the current season.")
            else:
                ...
                display_results(home, away, probs)

st.set_page_config(page_title="AI Sports", layout="wide")
st.title("🏆 Multi-League AI Match Predictor")

tabs = st.tabs(list(LEAGUES.keys()))

for i, (name, L_ID) in enumerate(LEAGUES.items()):
    with tabs[i]:
        render_league_tab(name, L_ID)
